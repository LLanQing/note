* **一.变量**
    ...
    * 5）[指针和引用的区别](https://www.nowcoder.com/ta/nine-chapter/review?page=11)？（代表意义、内存占用、初始化、指向是否可改、能否为空）
     # C++指针和引用的区别
      ...
      C++ 中指针和引用之间有以下几个主要的区别:
      
      1. **定义方式**:
         - 指针是通过 `*` 操作符定义的变量,用于存储另一个变量的地址。
         - 引用是通过 `&` 操作符定义的变量,它只是原变量的一个别名。
      
      2. **语法**:
         - 指针使用 `*` 操作符访问其指向的变量。
         - 引用直接使用变量名访问其值,就像访问原变量一样。
      
      3. **赋值**:
         - 指针可以在定义后改变其指向的对象。
         - 引用在定义后就不能再指向其他对象,它始终指向初始化时指定的对象。
      ...
      5. **空值表示**:
         - 指针可以指向 `NULL` 或者 `0` 来表示空值。
         - 引用不能指向空值,必须初始化为一个有效的对象。
      
      6. **参数传递**:
         - 可以使用指针作为函数参数,以间接操作原变量。
         - 可以使用引用作为函数参数,直接操作原变量。
      
      总的来说,指针提供了更灵活的内存管理能力,而引用则提供了更安全和方便的变量访问方式。在 C++ 编程中,合理地使用指针和引用可以提高程序的性能和可读性。
      ...
      1. **引用是变量的别名**:
         - 引用在定义时会绑定到一个已经存在的变量上,成为该变量的另一个名字。
         - 引用本身不是一个独立的内存单元,它只是原变量的一个别名。
      
      2. **编译器的优化**:
         - 在编译时,编译器会将引用替换为原变量的名称,因此引用不需要额外的内存空间。
         - <font color="red">编译器会根据上下文将引用优化为直接访问原变量。</font>
      
      3. **避免间接访问开销**:
         - 使用指针需要通过间接访问来获取数据,会产生一些开销。
         - 而引用可以直接访问原变量,避免了这种间接访问的开销。
      
      4. **更好的性能**:
         - 由于引用不需要额外内存空间,且访问效率更高,因此在某些情况下引用的性能会更好。
         - 特别是在函数参数传递和返回值使用引用时,可以避免不必要的内存拷贝。
      
      总的来说,引用作为原变量的别名,在编译时会被优化为直接访问原变量的形式,从而避免了额外的内存占用和间接访问开销,提高了程序的运行效率。这是引用相比于指针的一个重要优势。
